...
def generate_clustered_targets_scenario(
    ...
    while target_id < num_targets:
        pos = (random_state.randint(0, grid_size[0]), 
               random_state.randint(0, grid_size[1]))
        if pos not in occupied:
            severity = random_state.randint(1, 11)
            targets.append({
    ...
    while vehicle_id < num_vehicles:
        ...
        if 0 <= pos[0] < grid_size[0] and 0 <= pos[1] < grid_size[1] and pos not in occupied:
            vehicles.append({
                'id': vehicle_id,
                'pos': pos,
                'target_id': None,
...
def generate_diagonal_wall_scenario(
    ...
    while vehicle_id < num_vehicles:
        ...
        offset_c = random_state.randint(-2, 3)
        pos = (center[0] + offset_r, center[1] + offset_c)
        if (0 <= pos[0] < grid_size[0] and 
            0 <= pos[1] < grid_size[1] and 
            pos not in occupied):
            vehicles.append({
                'id': vehicle_id,
                'pos': pos,
